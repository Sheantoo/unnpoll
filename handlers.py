...
questions = [arr[0]['questions'],arr[1]['questions'],arr[2]['questions'],arr[3]['questions']]        
...
async def start_quiz(chat_id, bot, lenq):
    await ask_question(chat_id, bot, lenq)

async def ask_question(chat_id, bot, lenq):
    if lenq > 1:
        
        current_question = questions[int(lenq)-1][0]
...
async def process_item_callback(callback_query: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    lenq = len(questions)
    await start_quiz(chat_id, bot, lenq)
...
async def process_text_answer(message: types.Message, state: FSMContext):
    ...
    answers.append({"question": questions[len(answers)]["questionText"], "answer": message.text})
    await state.update_data(answers=answers)
    await ask_question(chat_id, bot)

@router.callback_query(QuizStates.waiting_for_answer)
async def process_callback(callback_query: types.CallbackQuery, state: FSMContext):
    chat_id = callback_query.message.chat.id
    data = callback_query.data
    current_state = await state.get_state()

    if data == "confirm_checkbox" or data == "confirm_radio":
        state_data = await state.get_data()
        selected_options = state_data.get("selected_options", [])
        answers = state_data.get("answers", [])
        answers.append({"question": questions[len(answers)]["questionText"], "answer": selected_options})
        await state.update_data(answers=answers, selected_options=[])
        await callback_query.answer("Ответ подтвержден!")
        await ask_question(chat_id)
    else:
        state_data = await state.get_data()
        selected_options = state_data.get("selected_options", [])
        if data in selected_options:
            selected_options.remove(data)
        else:
            selected_options.append(data)
        await state.update_data(selected_options=selected_options)
        await callback_query.answer(f"Вы выбрали: {', '.join(selected_options)}")
